...
const translateSchema = z
  .object({
    word: z
      ...
      .trim(),
    sourceLang: z.string().length(2, 'Language code must be 2 characters').uppercase(),
    targetLang: z.string().length(2, 'Language code must be 2 characters').uppercase(),
...
const browseSchema = z
  .object({
    ...
    format: z
      .enum(['TV', 'Movie', 'OVA', 'ONA', 'Manga', 'Unknown'], {
    ...
    status: z
      ...
      .optional(),
    sort: z.string().max(50, 'Invalid sort parameter').optional().default('TRENDING_DESC'),
    year: z.string().max(4, 'Year must be 4 characters').optional().default(''),
    limit: z
      .number()
...
const suggestedSchema = z
  .object({
    userId: z.string().min(20, 'Invalid user ID format').max(50, 'Invalid user ID format'),
    limit: z
      .number()
      .int('Limit must be an integer')
      .min(1, 'Limit must be at least 1')
...
const recaptchaSchema = z
  .object({
    token: z.string().min(20, 'Invalid reCAPTCHA token').max(1000, 'Invalid reCAPTCHA token'),
  })
  .strict();
