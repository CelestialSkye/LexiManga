...
export const getResponsiveImageSize = () => {
  ...
  // Medium screens (768px - 1199px)
    ...
    const scale = Math.max(0.8, Math.min(1.0, (screenWidth - breakpoints.mobile) / (breakpoints.desktop - breakpoints.mobile)));
    return { 
...
export const getResponsiveTextSize = () => {
  ...
  const baseHeadingSize = Math.max(heading.min, Math.min(heading.max, screenWidth / heading.divisor));
  const baseParagraphSize = Math.max(paragraph.min, Math.min(paragraph.max, screenWidth / paragraph.divisor));
  ...
  const maxWidthValue = Math.max(maxWidth.min, Math.min(maxWidth.max, screenWidth / maxWidth.divisor));
