...
// get all words
export const useVocabWords = (uid) => {
    return useQuery({
...
// get words by manga
export const useVocabWordsByManga = (uid, mangaId) => {
    return useQuery({
...
export const useAddVocabWord = () => {
    ...
    return useMutation({
        mutationFn: ({ uid, mangaId, wordId, wordData }) => saveVocabWord(uid, mangaId, wordId, wordData),
        onSuccess: (data, variables) => {
            queryClient.invalidateQueries(['vocabWord', variables.uid, variables.mangaId, variables.wordId]);
...
export const useUpdateVocabWord = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ uid, mangaId, wordId, wordData }) => updateVocabWord(uid, mangaId, wordId, wordData),
        onSuccess: (data, variables) => {
            queryClient.invalidateQueries(['vocabWord', variables.uid, variables.mangaId, variables.wordId]);
            queryClient.invalidateQueries(['vocabWords', variables.uid]);
            queryClient.invalidateQueries(['vocabWordsByManga', variables.uid, variables.mangaId]);
